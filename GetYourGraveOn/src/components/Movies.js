...
function Movies() {
  ...
  function getMotd() {
    ...
    axios
      ...
      .then(searchResults => {
        ...
        const moviePoster = (
          <Poster posterOnly={true} movies={movies} onClick={addToDisplay} />
        );
        setMotd(moviePoster);
      })
      .catch(err => {
  ...
  function getFlipPoster(id) {
    ...
    axios
      ...
      .then(searchResults => {
        setSelectedPoster(
          <Poster posterOnly={false} movies={searchResults.data} />
        );
      })
      .catch(err => {
        console.error(
          "Error getting data from back end from flip movie: ",
          err
        );
      });
  }
    ...
    axios
      ...
      .then(searchResults => {
        ...
        movieRows.length > 0
          ...
          : setMovieRows(<h1 className="noMatch">No Matches Found</h1>);
      })
      .catch(error => {
        console.error(
          "Perform search Error coming from backend search:   ",
          error
        );
      });
  }
    ...
    <Container fluid={true} id="movies_container">
      ...
      <Row id="movieDisplayRow">
        ...
        <Col xl={4} sm={12} id="test">
          <div id="posterSelected">{selectedPoster}</div>
          <input
            type="text"
